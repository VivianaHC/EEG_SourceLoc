function EEGviewer(varargin)
% EEGviewer eegdisplay(matrice,Fech) displays signals in time
% Command :
%           EEGviewer(matrix,Fech)
%        or EEGviewer(matrix,Fech,'file_names_electrodes')
%        or EEGviewer(matrix,Fech,cell_noms_electrodes)
%        or EEGviewer(matrix,Fech,64)
%
% Inputs:
%       matrix: The signals are by row (matrix is electrodes x time samples)
%       Fs: The second mandatory argument is the sampling frequency
%       The third argument is optional. If absent, the electrode names are the numbers
%       If you want to dsiplay the names, then give a cell array or a file
%       (names on different rows or separated by tabs or spaces). If you
%       give 64, then a standard EEG 10-10 is assumed:
...
elseif nargin==3
    ...
    if iscell(varargin{3})
        ...
        elseif exist(varargin{3},'file')
            ...
            disp('Check 3rd argument!!');
            return;
        end
    end
else
    disp('Check the number of inputs!!');
    return;
end
...
ancienne_valeur_nb_voies=16; % valeur par défaut du nombre de voies affichées

%amplitude_max=max(max(abs(sig)));
amplitude_max=500;

if size(sig,1)>length(electrodes)
    disp('Missing electrode names!!');
    return;
end
...
bouton_precedent = uicontrol(f,'Style','pushbutton','FontSize',14,'String','<<','Units','normalized',...
    'Position',[.03 .02 .04 .04],'Callback',{@precedent},'ToolTipString','back 10 seconds','ForegroundColor',[.1 .1 .9]);

bouton_suivant = uicontrol(f,'Style','pushbutton','FontSize',14,'String','>>','Units','normalized','Position',...
    [.93 .02 .04 .04],'Callback',{@suivant},'ToolTipString','forward 10 seconds','ForegroundColor',[.1 .1 .9]);

bouton_precedent_1 = uicontrol(f,'Style','pushbutton','FontSize',14,'String','<','Units','normalized',...
    'Position',[.085 .02 .04 .04],'Callback',{@precedent_1},'ToolTipString','back 1 second','ForegroundColor',[.1 .1 .9]);

bouton_suivant_1 = uicontrol(f,'Style','pushbutton','FontSize',14,'String','>','Units','normalized','Position',...
    [.875 .02 .04 .04],'Callback',{@suivant_1},'ToolTipString','forward 1 second','ForegroundColor',[.1 .1 .9]);
...
bouton_courbes_suivantes = uicontrol(f,'Style','pushbutton','FontSize',10,'FontWeight','bold','String','\/','Units','normalized','Position',...
    [.93 .12 .04 .04],'Callback',{@courbes_suiv},'ToolTipString','next electrodes','ForegroundColor',[.0 .3 .0]);

texte_facteur=uicontrol(f,'Style','text','FontSize',8,'String','scaling factor','Units','normalized',...
...
uicontrol(f,'Style','text','FontSize',8,'String','number of channels','Units','normalized',...
    'Position',[.91 .5 .085 .03],'BackgroundColor',couleur,'HorizontalAlignment','center','ToolTipString','Number of channels to plot');
...
bouton_affichage_temps_total=uicontrol(f,'Style','pushbutton','FontSize',10,'String','Display all','Units','normalized','Position',...
...
texte_affichage_perso=uicontrol(f,'Style','text','FontSize',8,'String','Display from','Units','normalized',...
...
bouton_affperso_ok=uicontrol(f,'Style','pushbutton','FontSize',8,'FontWeight','bold','String','OK','Units','normalized','Position',...
    [.355 .02 .04 .04],'Callback',{@affich_perso},'ToolTipString','validate','ForegroundColor',[.9 .1 .1]);
...
function trace(prem_voie,ti,nb_voies_affichees,tfp)
    ti=round(ti);
    if strcmp(get(bouton_affichage_temps_total,'String'),'Display 20s') % Attention : c'est le contraire qui s'affiche
    ...
    if strcmp(get(bouton_affichage_temps_total,'String'),'Display 20s')&&affp==0 % Attention : c'est le contraire qui s'affiche
    ...
    grid on;
    if strcmp(get(bouton_affichage_temps_total,'String'),'Display all')&&affp==0
        set(gca,'XTick',fix(Temps_sig):2:Temps_sig+22);
    end
    xlabel('Time (s)');
    hold off;
...
function courbes_suiv(hObject, eventdata, handles)
    ...
    if affp==0
        if strcmp(get(bouton_affichage_temps_total,'String'),'Display 20s')
...
function courbe_entiere(hObject, eventdata, handles)
    affp=0;
    if strcmp(get(hObject,'String'),'Display all')
        set(hObject,'String','Display 20s');
        set(bouton_precedent,'Enable','off');
        set(bouton_suivant,'Enable','off');
        set(bouton_precedent_1,'Enable','off');
        set(bouton_suivant_1,'Enable','off');
        trace(Voie_affich,1,str2num(get(val_nb_voies,'String')),0);
    elseif strcmp(get(hObject,'String'),'Display 20s')
        set(hObject,'String','Display all');
        if size(sig,2)>=Fech
